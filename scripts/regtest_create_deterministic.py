...
async def generate_deterministic_test_vector():
    ...
    keys = {
        "genesisKey": {"pk": initial_pk.sec().hex(), "sk": initial_sk.hex()},
    }
    ...
    for _beacon_id, beacon_manager in did_manager.beacon_managers.items():
    ...
    beacon_manager = did_manager.add_beacon_manager(
        new_beacon_service.id, new_beacon_sk, beacon_script
    )
    await fund_regtest_beacon_address(bitcoin_rpc, beacon_manager)
    ...
    second_updated_doc = await did_manager.finalize_update_payload(
        updater, vm_id, initial_sk, new_beacon_service.id
    )

    keys[new_beacon_service.id] = {"pk": new_beacon_sk.point.sec().hex(), "sk": new_beacon_sk.hex()}
    keys[new_vm.id] = {"pk": new_vm_pk.sec().hex(), "sk": new_vm_sk.hex()}
    ...
    resolution_options = {"sidecarData": did_manager.get_sidecar_data()}
