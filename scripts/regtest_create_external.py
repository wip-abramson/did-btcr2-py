...
async def generate_external_test_vector():
    ...
    keys = {
        vm.id: {"pk": auth_pk.sec().hex(), "sk": auth_sk.hex()},
        cap_vm.id: {"pk": cap_pk.sec().hex(), "sk": cap_sk.hex()},
    }
    ...
    beacon_id = DIDUrl.unparse(
        identifier, beacon_service.id.path, beacon_service.id.query, beacon_service.id.fragment
    )
